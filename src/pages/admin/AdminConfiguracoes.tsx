...
const GerenteFormDialog = memo(function GerenteFormDialog({
  open, onClose, onOpenTempPassword, initial,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        ...
        toast({ title: 'Gerente criado com sucesso.', description: creationMode === 'invite' ? `Um e-mail foi enviado para ${email.trim()}.` : 'Defina a senha temporária na próxima etapa.' });
        queryClient.invalidateQueries({ queryKey: ['gerentes'] });
        onClose();
        // If temp-password mode, open the temp password modal
        if (creationMode === 'temp-password' && data?.user_id && onOpenTempPassword) {
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          {/* Creation mode selector (only for new gerentes) */}
          {!isEdit && (
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                {creationMode === 'invite'
                  ? 'O gerente receberá um e-mail com link para criar sua senha.'
...
const GerenteRow = memo(function GerenteRow({
  gerente, onEdit, onToggleStatus, onResetPassword, onSetTempPassword, onDelete, actionLoading,
}: GerenteRowProps) {
  const isActive = gerente.status === 'ativo';
  const busy = actionLoading === gerente.id;
  const hasTempPassword = !!gerente.must_change_password;
...
function GerentesTab() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from('profiles')
        .select('id, name, email, cargo, status, last_sign_in_at, must_change_password', { count: 'exact' })
        .eq('role', 'gerente')
        .order('name');
    ...
    <div className="space-y-5">
            ...
            {isLoading ? (
              ...
              gerentes.map(gerente => (
                <GerenteRow
                  ...
                  onSetTempPassword={() => setTempPasswordTarget({ id: gerente.id, name: gerente.name })}
      ...
      {tempPasswordTarget && (
        <TempPasswordModal
          ...
          onSuccess={() => queryClient.invalidateQueries({ queryKey: ['gerentes'] })}
        />
      )}
    </div>
  );
