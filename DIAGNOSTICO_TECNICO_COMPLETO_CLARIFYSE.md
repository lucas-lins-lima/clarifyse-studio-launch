...
| C5 | **Portal do Cliente** | As rotas do cliente (`/cliente/*`) estão definidas em arquivos mas **não estão registradas no App.tsx**. O portal do cliente é inacessível via navegação. | **Portal inacessível** |
| C6 | **Portal do Gerente** | As rotas do gerente (`/gerente/*`) também **não estão registradas no App.tsx**. | **Painel inacessível** |
...
| Funcionalidade | Status | Observação |
|---|---|---|
| Portal exclusivo por projeto | ⚠️ Parcial | Código existe mas rotas não registradas no App.tsx |
