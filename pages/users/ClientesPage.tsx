...
const ClientFormDialog = memo(function ClientFormDialog({
  open, onClose, onSuccess, initial, isAdmin,
...
const ClientRow = memo(function ClientRow({
  client, isAdmin, onEdit, onViewProjects, onToggleStatus, onResetPassword, onDelete, actionLoading,
}: ClientRowProps) {
...
export default function ClientesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from('profiles')
        .select('id, name, email, empresa, status, last_sign_in_at', { count: 'exact' })
        .eq('role', 'cliente')
        .order('name');
  ...
  async function exportExcel() {
    ...
    try {
      let q = supabase
        .from('profiles')
        .select('id, name, email, empresa, status, last_sign_in_at')
        .eq('role', 'cliente')
        .order('name');
      if (statusFilter !== 'todos') q = q.eq('status', statusFilter);
      if (search.trim()) {
