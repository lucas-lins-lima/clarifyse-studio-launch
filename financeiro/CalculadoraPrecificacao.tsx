...
export function CalculadoraPrecificacao() {
  ...
  const [customCPR, setCustomCPR] = useState<number | null>(null);
  const [partnerCPRInfo, setPartnerCPRInfo] = useState<{ average: number | null; count: number }>({ average: null, count: 0 });
  ...
  // Fetch panel partner CPRs
  const { data: partnerCPRs } = useQuery({
    queryKey: ['panel-partner-cpr'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('panel_partner_cpr')
        .select('perfil, faixa_perguntas, cpr_valor')
        .eq('cpr_valor', '>', 0)
        .in('cpr_valor', Array(1000).fill('IS NOT NULL'));

      if (error) throw error;
      return (data ?? []) as Array<{ perfil: string; faixa_perguntas: string; cpr_valor: number }>;
    },
  });

  // Calculate average CPR from partners for selected profile
  const getAverageCPR = (perfil: string, faixa: string): { average: number | null; count: number } => {
    if (!partnerCPRs || partnerCPRs.length === 0) {
      return { average: null, count: 0 };
    }

    const matching = partnerCPRs.filter(
      (cpr) => cpr.perfil === perfil && cpr.faixa_perguntas === faixa && cpr.cpr_valor && cpr.cpr_valor > 0
    );
    ...
    const average = matching.reduce((sum, cpr) => sum + (cpr.cpr_valor || 0), 0) / matching.length;
    return { average, count: matching.length };
  };

  // Update partner CPR when profile/range changes
  React.useEffect(() => {
    if (metodologia === 'quantitativa') {
      const cprInfo = getAverageCPR(tipoPerfil, faixaQuestoes);
      setPartnerCPRInfo(cprInfo);
      if (cprInfo.average) {
        setCustomCPR(cprInfo.average);
      }
    }
  }, [tipoPerfil, faixaQuestoes, partnerCPRs, metodologia]);
  ...
  const calculation = useMemo(() => {
    ...
    if (metodologia === 'quantitativa') {
      ...
      // Use custom CPR (from partners) if available, otherwise use defaults
      const cprKey = `cpr_${tipoPerfil}_${faixaQuestoes}`;
      const cpr = customCPR ?? defaults[cprKey] ?? 0;
          ...
          <div className="space-y-4">
            ...
            {metodologia === 'quantitativa' ? (
              <>
                ...
                {/* CPR from partners */}
                {partnerCPRInfo.average && (
                  <div className="space-y-1.5 p-3 bg-teal-50 dark:bg-teal-950/30 rounded-lg border border-teal-200 dark:border-teal-800">
                    <div className="flex items-center gap-2">
                      <Info className="h-4 w-4 text-teal-600" />
                      <p className="text-xs font-medium text-teal-700 dark:text-teal-300">
                        Média dos parceiros: {formatCurrency(partnerCPRInfo.average)}
                        <span className="ml-1 text-teal-600 dark:text-teal-400">({partnerCPRInfo.count} parceiro{partnerCPRInfo.count !== 1 ? 's' : ''})</span>
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  <Label className="flex items-center gap-1">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-[250px]">
                        <p className="text-xs">
                          Deixe em branco para usar a média dos parceiros ou valor padrão.
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  </Label>
                  <Input
                    ...
                    min={0}
                    step={0.01}
                    placeholder={partnerCPRInfo.average ? `${partnerCPRInfo.average.toFixed(2)}` : 'Insira o CPR manualmente'}
                    value={customCPR || ''}
                    onChange={(e) => setCustomCPR(e.target.value ? parseFloat(e.target.value) : null)}
                  />
                </div>
